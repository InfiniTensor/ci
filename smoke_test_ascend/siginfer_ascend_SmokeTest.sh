...
curr_dir=/home/s_limingge/smoke_test
        ...
        for ((j=1; j<=1; j=j+1)); do
            for item in "${model_list[@]}"; do
                ...
                for ip in ${server_list[@]}; do
                    ...
                    if [ $ip == "10.9.1.6" ]; then
                        sshpass -p 's_limingge' ssh -o ConnectionAttempts=3 s_limingge@10.9.1.6 /home/s_limingge/job_executor_for_SmokeTest.sh $model $gpu_quantity $use_prefix_cache_flag $option $swap_space $local_master_ip $seq_num $job_count $version &
                        pid_map[$!]="10.9.1.6"
                    else
                        ssh -o ConnectionAttempts=3 s_limingge@$ip /home/s_limingge/job_executor_for_SmokeTest.sh $model $gpu_quantity $use_prefix_cache_flag $option $swap_space $local_master_ip $seq_num $job_count $version &
                        pid_map[$!]=$ip
                    fi
                ...
                while (( remaining > 0 )); do
                    ...
                    if [ $err -ne 0 ]; then
                        ...
                        for ip in ${server_list[@]}; do
                            if [ $ip == "10.9.1.6" ]; then
                                sshpass -p 's_limingge' ssh -o ConnectionAttempts=3 s_limingge@10.9.1.6 docker stop siginfer_ascend_SmokeTest_${job_count}
                                sshpass -p 's_limingge' ssh -o ConnectionAttempts=3 s_limingge@10.9.1.6 docker rm siginfer_ascend_SmokeTest_${job_count}
                            else
                                ssh -o ConnectionAttempts=3 s_limingge@$ip docker stop siginfer_ascend_SmokeTest_${job_count}
                                ssh -o ConnectionAttempts=3 s_limingge@$ip docker rm siginfer_ascend_SmokeTest_${job_count}
                            fi
                        done
                        success=1
                        break
                ...
                # 开始执行测试
                pytest --env ${npu_server_list[${server_list[0]}]} chat --alluredir report > "$curr_dir/$filename"
                pytest --env ${npu_server_list[${server_list[0]}]} text --alluredir report >> "$curr_dir/$filename"
