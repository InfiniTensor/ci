...
def extract_test_summary(log_file_path):
    try:
        ...
        if index == 0:
            return "No test summary info found in the log file. Please check status of test tasks if it's been successfully executed."
        elif index == 1:
            return "\n".join(str(item) for item in summary_lines[0])
        else:
            return "\n".join(str(item) for item in summary_lines[0]) + "\n\n" + "\n".join(str(item) for item in summary_lines[1])
...
def save_summary_to_log(version_text, model_name, summary_text):
    if not summary_text:
        print("No summary to send.")
        return
...
def main():
    ...
    summary = extract_test_summary(log_file_path)
    # send_summary_to_server("docker.xcoresigma.com/docker/siginfer-aarch64-ascend:" + docker_image_version, model_name, summary)
    save_summary_to_log("docker.xcoresigma.com/docker/siginfer-aarch64-ascend:" + docker_image_version, model_name, summary)
