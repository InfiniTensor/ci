...
full_model_list=(DeepSeek-R1:8:H20 DeepSeek-V3-0324:8:H20 Qwen3-235B-A22B-FP8:4:H20 DeepSeek-R1-Distill-Qwen-32B:2:A800 DeepSeek-R1-Distill-Llama-70B:4:A800 Meta-Llama-3.1-70B-Instruct:4:A800 Qwen2.5-32B-Instruct:2:A800 QwQ-32B:2:A800 Qwen2.5-32B-Instruct-AWQ:1:A800 QwQ-32B-AWQ:1:A800 DeepSeek-R1-Distill-Llama-70B:4:H100)
        ...
        for ((j=1; j<=2; j=j+1)); do
            for item in "${model_list[@]}"; do
                ...
                for ip in ${server_list[@]}; do
                    ...
                    ssh s_limingge@$ip /home/s_limingge/job_executor_for_StabilityTest.sh $model $gpu_quantity $use_prefix_cache_flag $option $swap_space $job_count $version &
                    pid_map[$!]=$ip
                ...
                while (( remaining > 0 )); do
                    ...
                    if [ $err -ne 0 ]; then
                        ...
                        # 启动失败，清理工作
                        for ip in ${server_list[@]}; do
                            ssh s_limingge@$ip docker stop siginfer_nvidia_StabilityTest_${job_count}
                            ssh s_limingge@$ip docker rm siginfer_nvidia_StabilityTest_${job_count}
                        done
                        success=1
                        break
                    fi
