...
# full_model_list=(DeepSeek-R1:8:H20 DeepSeek-R1-awq:8:H20 DeepSeek-R1-Distill-Qwen-32B:2:A800 Qwen2.5-72B-Instruct-AWQ:1:A800)
full_model_list=(DeepSeek-R1-Distill-Llama-70B:4:H100)
curr_dir=/home/s_limingge/stability_test_nvidia
...
for name in "${!H20_server_list[@]}"; do
    ...
    scp "${curr_dir}/job_executor_for_StabilityTest.sh" s_limingge@${H20_server_list[$name]}:/home/s_limingge
done

for name in "${!A800_server_list[@]}"; do
    echo "$name => ${A800_server_list[$name]}"
    scp "${curr_dir}/job_executor_for_StabilityTest.sh" s_limingge@${A800_server_list[$name]}:/home/s_limingge
done

for name in "${!H100_server_list[@]}"; do
    echo "$name => ${H100_server_list[$name]}"
    scp "${curr_dir}/job_executor_for_StabilityTest.sh" s_limingge@${H100_server_list[$name]}:/home/s_limingge
done
...
while true; do
    ...
    for item in "${GPU_resource_demand[@]}"; do
        ...
        if [ ${#servers[@]} -ge ${SERVER_QUANTITY} ]; then
            ...
            $curr_dir/siginfer_nvidia_StabilityTest.sh 0 "${servers[*]}" ${model} ${job_count} ${version} > $curr_dir/cron_job_${log_name_suffix}.log 2>&1 &
            tail -F /home/s_limingge/stability_test_nvidia/cron_job_${log_name_suffix}.log | grep --line-buffered -m 1 -E "按任意键结束"
            ((job_count++))
