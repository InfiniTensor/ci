            ...
            for item in "${model_list[@]}"; do
                ...
                for ip in ${server_list[@]}; do
                    ...
                    if [ $TEST_TYPE == "Smoke" ]; then
                        if [ $ip == "10.9.1.6" ]; then
                            sshpass -p 's_limingge' ssh -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 s_limingge@10.9.1.6 /home/s_limingge/job_executor_for_${TEST_TYPE}Test.sh $model $gpu_quantity $use_prefix_cache_flag $option $swap_space $local_master_ip $seq_num $job_count $version > $curr_dir/$filename &
                            pid_map[$!]="10.9.1.6"
                        else
                            ssh -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 s_limingge@$ip /home/s_limingge/job_executor_for_${TEST_TYPE}Test.sh $model $gpu_quantity $use_prefix_cache_flag $option $swap_space $local_master_ip $seq_num $job_count $version > $curr_dir/$filename &
                            pid_map[$!]=$ip
                        fi
                    else
                        if [ $ip == "10.9.1.6" ]; then
                ...
                if [ $TEST_TYPE == "Smoke" ]; then
                    ...
                    # 获取模型启动命令，并做为参数传入
                    launch_cmd=`tail -2 $curr_dir/$filename`
                    # ......
