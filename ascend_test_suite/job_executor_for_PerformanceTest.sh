...
if [ $MODEL == "DeepSeek-R1-AWQ" ]; then
    echo "SIG_LOG_LEVEL='warn,console_logger=info' ./start.sh --model DeepSeek-R1-AWQ --tokenizer /home/weight/DeepSeek-R1-AWQ/ -tp 8 --port $PORT --platform-type ascend --block-size 128 --weight-dtype=FP16 --schedule-policy $SCHEDULE_POLICY --max-num-batched-tokens 8192 --master-addr $MASTER_IP --master-port $MASTER_PORT --nnodes 1 --node-rank $NODE_RANK --quantization awq $SWAP_SPACE_OPTION --tool-call-parser deepseek_v3 --prometheus-port $PROMETHEUS_PORT"
    ...
    echo "SIG_LOG_LEVEL='warn,console_logger=info' ./start.sh --model DeepSeek-R1 --tokenizer /home/weight/DeepSeek-R1/ -tp 16 --port $PORT --platform-type ascend --block-size 128 --weight-dtype=FP16 --schedule-policy $SCHEDULE_POLICY --max-num-batched-tokens 2048 --master-addr $MASTER_IP --master-port $MASTER_PORT --nnodes 2 --node-rank $NODE_RANK $SWAP_SPACE_OPTION --tool-call-parser deepseek_v3 --prometheus-port $PROMETHEUS_PORT"
    ...
    echo "SIG_LOG_LEVEL='warn,console_logger=info' ./start.sh --model DeepSeek-R1-W8A8 --tokenizer /home/weight/DeepSeek-R1-Channel-INT8/ -tp 16 --port $PORT --platform-type ascend --block-size 128 --weight-dtype=FP16 --schedule-policy $SCHEDULE_POLICY --gpu-memory-utilization 0.97 --max-num-batched-tokens 2048 --master-addr $MASTER_IP --master-port $MASTER_PORT --nnodes 2 --node-rank $NODE_RANK $SWAP_SPACE_OPTION --tool-call-parser deepseek_v3 --tokens-per-prediction 2 --prometheus-port $PROMETHEUS_PORT"
    ...
    echo "SIG_LOG_LEVEL='warn,console_logger=info' ./start.sh --model Qwen3-235B-A22B --tokenizer /home/weight/Qwen3/Qwen3-235B-A22B -tp 8 --block-size 128 --weight-dtype=FP16 --schedule-policy $SCHEDULE_POLICY --max-num-batched-tokens 8192 --port $PORT --master-addr $MASTER_IP --master-port $MASTER_PORT --nnodes 1 --node-rank $NODE_RANK --gpu-memory-utilization 0.98 --platform-type ascend $SWAP_SPACE_OPTION --tool-call-parser hermes --prometheus-port $PROMETHEUS_PORT"
    EXEC_COMMAND+=" --model Qwen3-235B-A22B --tokenizer /home/weight/Qwen3/Qwen3-235B-A22B -tp 8 --block-size 128 --weight-dtype=FP16 --schedule-policy $SCHEDULE_POLICY --max-num-batched-tokens 8192 --port $PORT --master-addr $MASTER_IP --master-port $MASTER_PORT --nnodes 1 --node-rank $NODE_RANK --gpu-memory-utilization 0.98 --platform-type ascend $SWAP_SPACE_OPTION --tool-call-parser hermes --prometheus-port $PROMETHEUS_PORT > $LOG_NAME 2>&1 &"
