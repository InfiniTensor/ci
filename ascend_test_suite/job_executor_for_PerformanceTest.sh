...
if [ $MODEL == "DeepSeek-R1-AWQ" ]; then
    echo "SIG_LOG_LEVEL='warn,console_logger=info' ./start.sh --model DeepSeek-R1-AWQ --tokenizer /home/weight/DeepSeek-R1-AWQ/ -tp 8 --port $PORT --platform-type ascend --block-size 128 --weight-dtype=FP16 --schedule-policy $SCHEDULE_POLICY --max-num-batched-tokens 8192 --master-addr $MASTER_IP --master-port $MASTER_PORT --nnodes 1 --node-rank $NODE_RANK --quantization awq $SWAP_SPACE_OPTION --tool-call-parser deepseek_v3 $USE_PREFIX_CACHE --prometheus-port $PROMETHEUS_PORT"
