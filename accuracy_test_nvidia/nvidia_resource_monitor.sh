...
while true; do
    ...
    for item in "${GPU_resource_demand[@]}"; do
        ...
        if [ ${#servers[@]} -ge ${SERVER_QUANTITY} ]; then
            ...
            echo
            /home/s_limingge/accuracy_test_nvidia/siginfer_nvidia_AccuracyTest.sh 0 "${servers[*]}" ${model} ${job_count} ${version} > /home/s_limingge/accuracy_test_nvidia/cron_job_${log_name_suffix}_${job_count}.log 2>&1 &
            tail -F /home/s_limingge/accuracy_test_nvidia/cron_job_${log_name_suffix}_${job_count}.log | grep --line-buffered -m 1 -E "开始执行模型精度测试任务"
            ((job_count++))
