...
if [ -z $VERSION ]; then
    ...
    TAGS=$(/home/zkjh/jfrog rt curl \
        --server-id=my-jcr \
        /api/docker/docker-local/v2/infiniTensor-aarch64-metax/tags/list \
    | jq -r '.tags[]' | sort)
    ...
    created=$(/home/zkjh/jfrog rt curl \
        --server-id=my-jcr \
        /api/storage/docker-local/infiniTensor-aarch64-metax/$tag \
        | jq -r '.created')
    echo "$tag $created"
    done > tag_dates.txt
...
docker pull docker.xcoresigma.com/docker/infiniTensor-aarch64-metax:$LATEST_TAG
if [ $? -ne 0 ]; then
  exit 1;
fi

ret=`docker ps -a | grep infiniTensor_metax_<<<TEST_TYPE>>>_${SESSION_ID}_${JOB_COUNT}`
if [ $? -eq 0 ]; then
  docker stop infiniTensor_metax_<<<TEST_TYPE>>>_${SESSION_ID}_${JOB_COUNT}
  docker rm infiniTensor_metax_<<<TEST_TYPE>>>_${SESSION_ID}_${JOB_COUNT}
fi
...
if ! command -v npu-smi &> /dev/null; then
    echo "错误: npu-smi 未找到，请确保 metax 910B 驱动已安装"
    exit 1
fi
