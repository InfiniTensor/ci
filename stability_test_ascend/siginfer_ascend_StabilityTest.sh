        ...
        for ((j=1; j<=2; j=j+1)); do
            for item in "${model_list[@]}"; do
                ...
                for ip in ${server_list[@]}; do
                    ...
                    if [ $ip == "10.9.1.6" ]; then
                        sshpass -p 's_limingge' ssh -o ConnectionAttempts=3 s_limingge@10.9.1.6 /home/s_limingge/job_executor_for_stability.sh $model $gpu_quantity $use_prefix_cache_flag $option $swap_space $local_master_ip $seq_num $job_count $version &
                        pid_map[$!]="10.9.1.6"
                    else
                        ssh -o ConnectionAttempts=3 s_limingge@$ip /home/s_limingge/job_executor_for_stability.sh $model $gpu_quantity $use_prefix_cache_flag $option $swap_space $local_master_ip $seq_num $job_count $version &
                        pid_map[$!]=$ip
                    fi
                ...
                while (( remaining > 0 )); do
                    ...
                    if [ $err -ne 0 ]; then
                        ...
                        for ip in ${server_list[@]}; do
                            if [ $ip == "10.9.1.6" ]; then
                                sshpass -p 's_limingge' ssh -o ConnectionAttempts=3 s_limingge@10.9.1.6 docker stop siginfer_ascend_StabilityTest_${job_count}
                                sshpass -p 's_limingge' ssh -o ConnectionAttempts=3 s_limingge@10.9.1.6 docker rm siginfer_ascend_StabilityTest_${job_count}
                            else
                                ssh -o ConnectionAttempts=3 s_limingge@$ip docker stop siginfer_ascend_StabilityTest_${job_count}
                                ssh -o ConnectionAttempts=3 s_limingge@$ip docker rm siginfer_ascend_StabilityTest_${job_count}
                            fi
                        done
                        success=1
                        break
