...
# full_model_list=(DeepSeek-R1-Distill-Llama-8B:1 Meta-Llama-3.1-8B-Instruct:1 Qwen2.5-0.5B-Instruct:1 Qwen2.5-1.5B-Instruct:1 Qwen2.5-3B-Instruct:1 Qwen2.5-7B-Instruct:1 Qwen2.5-14B-Instruct:1 QwQ-32B:2 Qwen2.5-0.5B-Instruct-AWQ:1 Qwen2.5-1.5B-Instruct-AWQ:1 Qwen2.5-3B-Instruct-AWQ:1 Qwen2.5-7B-Instruct-AWQ:1 Qwen2.5-14B-Instruct-AWQ:1 Qwen2.5-32B-Instruct-AWQ:1 Qwen2.5-72B-Instruct-AWQ:2 QwQ-32B-AWQ:1 Qwen3-32B:2)
# full_model_list=(DeepSeek-R1-AWQ:8 DeepSeek-R1-W8A8:16 DeepSeek-R1-Distill-Qwen-32B:2 Qwen2.5-72B-Instruct-AWQ:2)
full_model_list=(DeepSeek-R1-Distill-Llama-70B:4 DeepSeek-R1-Distill-Qwen-32B:2 Qwen2.5-32B-Instruct-AWQ:1)
...
for name in "${!npu_server_list[@]}"; do
    ...
    if [ $name == 'aicc001' ]; then
        sshpass -p 's_limingge' scp "${curr_dir}/job_executor_for_stability.sh" s_limingge@${npu_server_list['aicc001']}:/home/s_limingge
    else
        scp "${curr_dir}/job_executor_for_stability.sh" s_limingge@${npu_server_list[$name]}:/home/s_limingge
    fi
done
...
while true; do
    ...
    for item in "${GPU_resource_demand[@]}"; do
        ...
        if [ ${#servers[@]} -ge ${SERVER_QUANTITY} ]; then
            ...
            /home/s_limingge/stability_test_ascend/siginfer_ascend_StabilityTest.sh 0 "${servers[*]}" ${model} ${job_count} ${version} > /home/s_limingge/stability_test_ascend/cron_job_${log_name_suffix}_${job_count}.log 2>&1 &
            tail -F /home/s_limingge/stability_test_ascend/cron_job_${log_name_suffix}_${job_count}.log | grep --line-buffered -m 1 -E "按任意键结束"
            ((job_count++))
