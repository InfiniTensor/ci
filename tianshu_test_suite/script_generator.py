...
def main():
    ...
    for row in sheet.iter_rows(min_row=2, max_row=row_count, values_only=True):
        ...
        result = re.sub(r"^.*docker\.xcoresigma\.com(\:\d+)?/docker/siginfer-x86_64-tianshu\:\S+", "", args)
        ...
        result = re.sub(r"--gpu-memory-utilization\s+\S+", "--gpu-memory-utilization 0.9", result)
    ...
    if not os.path.isfile(template_file_path):
        print(f"错误: 路径是目录，不是文件: {template_file_path}")
        print("目录内容:", os.listdir(template_file_path) if os.path.isdir(template_file_path) else "不是目录")
        sys.exit(1)
    try:
