    ...
    for item in "${GPU_resource_demand[@]}"; do
        ...
        if [ ${#servers[@]} -ge ${SERVER_QUANTITY} ]; then
            ...
            if [ $TEST_TYPE == "Stability" ]; then
                $curr_dir/siginfer_tianshu_test.sh 0 "${servers[*]}" ${item} ${job_count} ${TEST_TYPE} ${version} ${user} > $curr_dir/cron_job_${log_name_suffix}_${job_count}.log 2>&1 &
                pid_map[$!]=$item
                tail -F $curr_dir/cron_job_${log_name_suffix}_${job_count}.log | grep --line-buffered -m 1 -E "按任意键结束|测试全部完成"
            else
                $curr_dir/siginfer_tianshu_test.sh 1 "${servers[*]}" ${item} ${job_count} ${TEST_TYPE} ${version} ${user}> $curr_dir/cron_job_${log_name_suffix}_${job_count}.log 2>&1 &
                pid_map[$!]=$item
