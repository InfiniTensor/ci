    ...
    try:
        ...
        for line in lines:
            if "<<<generated source code>>>" in line:
                ...
                if test_type == "Service":
                    lines[line_num] = line.replace("<<<DOCKER_ARGS>>>", docker_args + " -e HIP_VISIBLE_DEVICES=$HIP_VISIBLE_DEVICES $DOCKER_IMAGE_URL python InfiniLM/python/infinilm/server/inference_server.py")
                else:
