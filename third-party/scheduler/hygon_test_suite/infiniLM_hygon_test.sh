...
cleanup_all_resources() {
    ...
    echo ""
    echo "=========================================="
    echo "${engine_type}_hygon_test.sh 退出，开始清理资源..."
    echo "=========================================="
    ...
    for ip in ${server_list[@]}; do
        ssh -p 14735 -q -o ConnectionAttempts=3 zkjh@$ip "
            name=${engine_type}_hygon_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count}
...
if [ $TEST_TYPE == "Service" ]; then
    ...
    if [ -z $version ]; then
        version=$(jfrog rt curl --server-id=my-jcr /api/docker/docker-local/v2/infiniLM-aarch64-hygon/tags/list | jq -r '.tags[]' \
                            | xargs -I% sh -c "echo -n \"%  \"; \
                                jfrog rt curl --server-id=my-jcr \
                                /api/storage/docker-local/infiniLM-aarch64-hygon/% \
    ...
    for item in "${model_list[@]}"; do
        ...
        for ip in ${server_list[@]}; do
            ...
            if [ $TEST_TYPE == "Smoke" ]; then
                ssh -p 14735 -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 zkjh@$ip chmod a+x /home/zkjh/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test_${OPTIONS}.sh
                ssh -p 14735 -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 zkjh@$ip /home/zkjh/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test_${OPTIONS}.sh $model $gpu_quantity "${OPTIONS}" $server_list_str $seq_num $job_count $gpu_model $session_id $version > "$curr_dir/logs/smoke/$session_id/${filename}_${seq_num}" &
                ssh_pid=$!
                pid_map[$ssh_pid]=$ip
                SSH_PID_MAP[$ssh_pid]=$ip
            else
                ssh -p 14735 -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 zkjh@$ip chmod a+x /home/zkjh/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test_${OPTIONS}.sh
                ssh -p 14735 -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 zkjh@$ip /home/zkjh/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test_${OPTIONS}.sh $model $gpu_quantity "${OPTIONS}" $server_list_str $seq_num $job_count $gpu_model $session_id $version &
                ssh_pid=$!
                pid_map[$ssh_pid]=$ip
                SSH_PID_MAP[$ssh_pid]=$ip
            fi
                ...
                if [ $err -ne 0 ]; then
                    ...
                    for ip in ${server_list[@]}; do
                        if [ $ENGINE_TYPE == "InfiniLM" ]; then
                            ssh -p 14735 -q -o ConnectionAttempts=3  zkjh@$ip docker stop infiniLM_hygon_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count}
                            ssh -p 14735 -q -o ConnectionAttempts=3 zkjh@$ip docker rm infiniLM_hygon_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count}
                        elif [ $ENGINE_TYPE == "vLLM" ]; then
                            ssh -p 14735 -q -o ConnectionAttempts=3 zkjh@$ip docker stop vllm_hygon_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count}
                            ssh -p 14735 -q -o ConnectionAttempts=3 zkjh@$ip docker rm vllm_hygon_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count}
                        fi
                    done
        ...
        if [ $TEST_TYPE == "Service" ]; then
            ssh -q -p 14735 -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 zkjh@$local_master_ip "
                    docker exec ${ENGINE_TYPE,}_hygon_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count} /bin/bash -c \"
        ...
        if [ $send_report -eq 1 ]; then
            ...
            if [ $TEST_TYPE == "Smoke" ]; then
                ...
                CI_report_folder="/artifacts/CI_hygon_test/${session_id}_hygon_npu_performancetest"
    ...
    for ip in ${server_list[@]}; do
        if [ $ENGINE_TYPE == "InfiniLM" ]; then
            ssh -q -p 14735 -o ConnectionAttempts=3 zkjh@$ip docker stop infiniLM_hygon_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count}
            ssh -q -p 14735 -o ConnectionAttempts=3 zkjh@$ip docker rm infiniLM_hygon_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count}
        fi
    done
