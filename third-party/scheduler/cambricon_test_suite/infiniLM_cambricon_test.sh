...
cleanup_all_resources() {
    ...
    for ip in ${server_list[@]}; do
        ssh -p 14735 -q -o ConnectionAttempts=3 zkjh@$ip "
            name=${engine_type}_cambricon_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count}
...
if [ $TEST_TYPE == "Service" ]; then
    ...
    if [ -z $version ]; then
        version=$(jfrog rt curl --server-id=my-jcr /api/docker/docker-local/v2/infiniLM-aarch64-cambricon/tags/list | jq -r '.tags[]' \
                            | xargs -I% sh -c "echo -n \"%  \"; \
                                jfrog rt curl --server-id=my-jcr \
                                /api/storage/docker-local/infiniLM-aarch64-cambricon/% \
                ...
                if [ $err -ne 0 ]; then
                    ...
                    for ip in ${server_list[@]}; do
                        if [ $ENGINE_TYPE == "InfiniLM" ]; then
                            ssh -p 14735 -q -o ConnectionAttempts=3  zkjh@$ip docker stop infiniLM_cambricon_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count}
                            ssh -p 14735 -q -o ConnectionAttempts=3 zkjh@$ip docker rm infiniLM_cambricon_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count}
                        elif [ $ENGINE_TYPE == "vLLM" ]; then
                            ssh -p 14735 -q -o ConnectionAttempts=3 zkjh@$ip docker stop vllm_cambricon_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count}
                            ssh -p 14735 -q -o ConnectionAttempts=3 zkjh@$ip docker rm vllm_cambricon_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count}
                        fi
                    done
