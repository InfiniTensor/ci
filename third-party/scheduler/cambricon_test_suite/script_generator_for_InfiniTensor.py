...
def main():
    ...
    if test_type != "Unit":
        ...
        for model in models:
            ...
            result = re.sub(
                r"^.*docker\.xcoresigma\.com/docker/infiniTensor-aarch64-cambricon\:\S+",
                "",
                args,
            )
        ...
        for line in lines:
            if "<<<generated source code>>>" in line:
                ...
                if test_type == "Unit":
                    ...
                    lines[line_num] = line.replace("<<<DOCKER_ARGS>>>", docker_args + " -e MACA_VISIBLE_DEVICES=$MACA_VISIBLE_DEVICES docker.infinitensor.com/docker/infiniTensor-aarch64-cambricon:$LATEST_TAG")
            line_num += 1
