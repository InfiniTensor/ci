...
cleanup_all_resources() {
    ...
    echo ""
    echo "=========================================="
    echo "${engine_type}_cambricon_test.sh 退出，开始清理资源..."
    echo "=========================================="
    ...
    # 3. 清理远程 Docker 容器
    for ip in ${server_list[@]}; do
        ssh -p 14735 -q -o ConnectionAttempts=3  zkjh@$ip "
            name=${engine_type}_cambricon_${TEST_TYPE}Test_${session_id}_${job_count}
...
handle_interrupt() {
    ...
    for remote_ip in "${!unique_ips[@]}"; do
        ...
        ssh -p 14735 -o ConnectionAttempts=3 -o ConnectTimeout=5  zkjh@$remote_ip "
...
if [ $TEST_TYPE == "Unit" ]; then
    ...
    ssh -p 14735 -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 zkjh@$ip chmod a+x /home/zkjh/${ENGINE_TYPE}_job_executor_for_UnitTest.sh
    ssh -p 14735 -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 zkjh@$ip /home/zkjh/${ENGINE_TYPE}_job_executor_for_UnitTest.sh $model $gpu_quantity $server_list_str 0 0 $session_id $version > "$curr_dir/logs/unit/$session_id/${filename}" &
    ssh_pid=$!
    pid_map[$ssh_pid]=$ip
    ...
    for ip in ${server_list[@]}; do
        if [ $ENGINE_TYPE == "InfiniTensor" ]; then
            ssh -p 14735 -q -o ConnectionAttempts=3  zkjh@$ip docker stop infiniTensor_cambricon_UnitTest_${session_id}_0
            ssh -p 14735 -q -o ConnectionAttempts=3  zkjh@$ip docker rm infiniTensor_cambricon_UnitTest_${session_id}_0
        fi
    done
...
if [ -z $version ]; then
    version=$(jfrog rt curl --server-id=my-jcr /api/docker/docker-local/v2/infiniTensor-aarch64-cambricon/tags/list | jq -r '.tags[]' \
                        | xargs -I% sh -c "echo -n \"%  \"; \
                            jfrog rt curl --server-id=my-jcr \
                            /api/storage/docker-local/infiniTensor-aarch64-cambricon/% \
...
for item in "${model_list[@]}"; do
    ...
    for ip in ${server_list[@]}; do
        ...
        if [ $TEST_TYPE == "Smoke" ]; then
            if [ $ENGINE_TYPE == "MindIE" ]; then
                ssh -p 14735 -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3  zkjh@$ip /home/zkjh/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test.sh $model $gpu_quantity $server_list_str $seq_num $job_count $session_id $version > "$curr_dir/logs/smoke/$session_id/${filename}_${seq_num}" &
            else
                ssh -p 14735 -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3  zkjh@$ip /home/zkjh/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test.sh $model $gpu_quantity $server_list_str $seq_num $job_count $session_id $version > "$curr_dir/logs/smoke/$session_id/${filename}_${seq_num}" &
            fi
            ssh_pid=$!
            ...
            if [ $ENGINE_TYPE == "MindIE" ]; then
                ssh -p 14735 -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3  zkjh@$ip /home/zkjh/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test.sh $model $gpu_quantity $server_list_str $seq_num $job_count $session_id $version &
            else
                ssh -p 14735 -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3  zkjh@$ip /home/zkjh/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test.sh $model $gpu_quantity $server_list_str $seq_num $job_count $session_id $version &
            fi
            ssh_pid=$!
    ...
    while (( remaining > 0 )); do
        ...
        if [ -v pid_map[$done_pid] ]; then
            ...
            if [ $err -ne 0 ]; then
                ...
                for ip in ${server_list[@]}; do
                    if [ $ENGINE_TYPE == "InfiniTensor" ]; then
                        ssh -p 14735 -q -o ConnectionAttempts=3  zkjh@$ip docker stop infiniTensor_cambricon_${TEST_TYPE}Test_${session_id}_${job_count}
                        ssh -p 14735 -q -o ConnectionAttempts=3  zkjh@$ip docker rm infiniTensor_cambricon_${TEST_TYPE}Test_${session_id}_${job_count}
                    elif [ $ENGINE_TYPE == "vLLM" ]; then
                        ssh -p 14735 -q -o ConnectionAttempts=3  zkjh@$ip docker stop vllm_cambricon_${TEST_TYPE}Test_${session_id}_${job_count}
                        ssh -p 14735 -q -o ConnectionAttempts=3  zkjh@$ip docker rm vllm_cambricon_${TEST_TYPE}Test_${session_id}_${job_count}
                    elif [ $ENGINE_TYPE == "MindIE" ]; then
                        ssh -p 14735 -q -o ConnectionAttempts=3  zkjh@$ip docker stop mindie_cambricon_${TEST_TYPE}Test_${session_id}_${job_count}
                        ssh -p 14735 -q -o ConnectionAttempts=3  zkjh@$ip docker rm mindie_cambricon_${TEST_TYPE}Test_${session_id}_${job_count}
                    fi
                done
    ...
    if [ $TEST_TYPE == "Smoke" ]; then
        ...
        echo "docker run --rm --name $container_name --security-opt seccomp=unconfined --ulimit nproc=8192:8192 --volume /data/shared/limingge/CI_Workspace_for_InfiniLM/ci_autotest/third-party/scheduler/cambricon_test_suite/report_${log_name_suffix}/$session_id:/test/report_${log_name_suffix} -e TASK_START_TIME=${log_name_suffix} -e OPENBLAS_NUM_THREADS=1 -e OMP_NUM_THREADS=1 -e MKL_NUM_THREADS=1 -e NUMEXPR_NUM_THREADS=1 -e JAVA_HOME=/usr/lib/jvm/java-11-openjdk-arm64 -e PYTHONUNBUFFERED=1 --entrypoint /test/start.sh openai:1110 --file $filename --email limingge@xcoresigma.com --env=${npu_server_list[${server_list[0]}]} --url http://${server_list[0]}:${server_port}/v1 --model=$model_name --gpu MLU590 --cmd \"$full_cmd\""
        DOCKER_API_VERSION=1.39 docker run --rm --name $container_name --security-opt seccomp=unconfined --ulimit nproc=8192:8192 --volume /data/shared/limingge/CI_Workspace_for_InfiniLM/ci_autotest/third-party/scheduler/cambricon_test_suite/report_${log_name_suffix}/$session_id:/test/report_${log_name_suffix} -e TASK_START_TIME=${log_name_suffix} -e OPENBLAS_NUM_THREADS=1 -e OMP_NUM_THREADS=1 -e MKL_NUM_THREADS=1 -e NUMEXPR_NUM_THREADS=1 -e JAVA_HOME=/usr/lib/jvm/java-11-openjdk-arm64 -e PYTHONUNBUFFERED=1 --entrypoint /test/start.sh openai:1110 --file $filename --email limingge@xcoresigma.com --env=${npu_server_list[${server_list[0]}]} --url http://${server_list[0]}:${server_port}/v1 --model=$model_name --gpu MLU590 --cmd "\"$full_cmd\"" 2>&1 &
        pid=$!
    ...
    if [ $send_report -eq 1 ]; then
        ...
        if [ $TEST_TYPE == "Smoke" ]; then
            ...
            CI_report_folder="/artifacts/CI_cambricon_test/${session_id}_cambricon_npu_performancetest"
