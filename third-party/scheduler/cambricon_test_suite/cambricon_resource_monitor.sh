...
search_servers() {
    ...
    for key in "${!npu_server_list[@]}"; do
        ...
        ssh -q -o -p 14735 ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 zkjh@${npu_server_list[$key]} "# 目标空闲 GPU 数量
...
if [ $TEST_TYPE == "Unit" ]; then
    while true; do
        ...
        if [ ${#servers[@]} -ge ${SERVER_QUANTITY} ]; then
            ...
            echo
            $curr_dir/infiniTensor_cambricon_test.sh 1 "${servers[*]}" ${model} 0 ${TEST_TYPE} ${ENGINE_TYPE} ${SESSION_ID} ${version} > $curr_dir/logs/unit/$SESSION_ID/cron_job_${log_name_suffix}_0.log 2>&1 &
            last_pid=$!
