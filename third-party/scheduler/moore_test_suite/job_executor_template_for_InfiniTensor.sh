...
if [ -z $VERSION ]; then
    ...
    TAGS=$(/home/zkjh/jfrog rt curl \
        --server-id=my-jcr \
        /api/docker/docker-local/v2/infiniTensor-aarch64-moore/tags/list \
    | jq -r '.tags[]' | sort)
    ...
    created=$(/home/zkjh/jfrog rt curl \
        --server-id=my-jcr \
        /api/storage/docker-local/infiniTensor-aarch64-moore/$tag \
        | jq -r '.created')
    echo "$tag $created"
    done > tag_dates.txt
...
if [ "<<<TEST_TYPE>>>" != "UnitTest" ]; then
    docker pull docker.xcoresigma.com/docker/infiniTensor-aarch64-moore:$LATEST_TAG
    if [ $? -ne 0 ]; then
        exit 1;
    fi
fi

ret=`docker ps -a | grep infiniTensor_moore_<<<TEST_TYPE>>>_${SESSION_ID}_${JOB_COUNT}`
if [ $? -eq 0 ]; then
  docker stop infiniTensor_moore_<<<TEST_TYPE>>>_${SESSION_ID}_${JOB_COUNT}
  docker rm infiniTensor_moore_<<<TEST_TYPE>>>_${SESSION_ID}_${JOB_COUNT}
fi
...
if ! command -v mthreads-gmi &> /dev/null; then
    echo "错误: mthreads-gmi 未找到，请确保 Moore 驱动已安装"
    exit 1
fi
...
EXEC_COMMAND+="-e MUSA_VISIBLE_DEVICES=$MTHREADS_VISIBLE_DEVICES "
