...
if [ -z $VERSION ]; then
    ...
    TAGS=$(/home/zkjh/jfrog rt curl \
        --server-id=my-jcr \
        /api/docker/docker-local/v2/infiniLM-aarch64-moore/tags/list \
    | jq -r '.tags[]' | sort)
    ...
    created=$(/home/zkjh/jfrog rt curl \
        --server-id=my-jcr \
        /api/storage/docker-local/infiniLM-aarch64-moore/$tag \
        | jq -r '.created')
    echo "$tag $created"
    done > tag_dates.txt
...
ret=`docker ps -a | grep infiniLM_moore_<<<TEST_TYPE>>>_${MODEL}_${OPTIONS}_${SESSION_ID}_${JOB_COUNT}`
if [ $? -eq 0 ]; then
  docker stop infiniLM_moore_<<<TEST_TYPE>>>_${MODEL}_${OPTIONS}_${SESSION_ID}_${JOB_COUNT}
  docker rm infiniLM_moore_<<<TEST_TYPE>>>_${MODEL}_${OPTIONS}_${SESSION_ID}_${JOB_COUNT}
fi
...
EXEC_COMMAND="docker run --name=infiniLM_moore_<<<TEST_TYPE>>>_${MODEL}_${OPTIONS}_${SESSION_ID}_${JOB_COUNT} "
