...
if [ $TEST_TYPE == "Service" ]; then
    ...
    if [ -z $version ]; then
        version=$(jfrog rt curl --server-id=my-jcr /api/docker/docker-local/v2/infiniLM-x86_64-nvidia/tags/list | jq -r '.tags[]' \
                | xargs -I% sh -c "echo -n \"%  \"; \
                    jfrog rt curl --server-id=my-jcr \
                    /api/storage/docker-local/infiniLM-x86_64-nvidia/% \
    ...
    for item in "${model_list[@]}"; do
        ...
        for ip in ${server_list[@]}; do
            ...
            if [ $TEST_TYPE == "Smoke" ]; then
                ...
                ssh -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 zkjh@$ip /home/zkjh/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test.sh $model $gpu_quantity "${TEST_PARAM}" $server_list_str $seq_num $job_count $gpu_model $session_id $version > "$curr_dir/logs/smoke/$session_id/${filename}_${seq_num}" &
                ssh_pid=$!
                pid_map[$ssh_pid]=$ip
                SSH_PID_MAP[$ssh_pid]=$ip
            else
                ssh -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 zkjh@$ip chmod a+x /home/zkjh/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test.sh
                ssh -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 zkjh@$ip /home/zkjh/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test.sh $model $gpu_quantity "${TEST_PARAM}" $server_list_str $seq_num $job_count $gpu_model $session_id $version &
                ssh_pid=$!
                pid_map[$ssh_pid]=$ip
                SSH_PID_MAP[$ssh_pid]=$ip
            fi
        ...
        if [ $TEST_TYPE == "Service" ]; then
            ...
            if [ $gpu_model == "H20" ]; then
                echo "docker run --rm --name $container_name --volume /data/shared/limingge/CI_Workspace_for_InfiniLM/ci_autotest/third-party/scheduler/nvidia_test_suite/report_${log_name_suffix}/$session_id:/test/report_${log_name_suffix} -e TASK_START_TIME=${log_name_suffix} --entrypoint /test/start.sh openai:1110 --file $filename --email limingge@xcoresigma.com --env=${H20_server_list[$local_master_ip]} --url http://${local_master_ip}:${server_port}/v1 --model=$model_name --gpu $gpu_model --cmd \"$full_cmd\""
                docker run --rm --name $container_name --volume /data/shared/limingge/CI_Workspace_for_InfiniLM/ci_autotest/third-party/scheduler/nvidia_test_suite/report_${log_name_suffix}/$session_id:/test/report_${log_name_suffix} -e TASK_START_TIME=${log_name_suffix} --entrypoint /test/start.sh openai:1110 --file $filename --email limingge@xcoresigma.com --env=${H20_server_list[$local_master_ip]} --url http://${local_master_ip}:${server_port}/v1 --model=$model_name --gpu $gpu_model --cmd "\"$full_cmd\"" 2>&1 &
                pid=$!
