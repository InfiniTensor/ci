...
if [ $TEST_TYPE == "Service" ]; then
    ...
    for option in "${TEST_PARAM[@]}"; do
        for item in "${model_list[@]}"; do
            ...
            filename="${log_name_suffix}_${model}.log"
            echo "开始测试模型: $model"
                    ...
                    if [ $err -ne 0 ]; then
                        ...
                        for ip in ${server_list[@]}; do
                            if [ $ENGINE_TYPE == "InfiniLM" ]; then
                                ssh -q -o ConnectionAttempts=3 zkjh@$ip docker stop infiniLM_nvidia_${TEST_TYPE}Test_${session_id}_${job_count}
                                ssh -q -o ConnectionAttempts=3 zkjh@$ip docker rm infiniLM_nvidia_${TEST_TYPE}Test_${session_id}_${job_count}
