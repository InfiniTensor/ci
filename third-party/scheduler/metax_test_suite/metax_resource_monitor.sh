...
for name in "${!npu_server_list[@]}"; do
    ...
    scp "${curr_dir}/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test_${TEST_PARAM// /_}.sh" zkjh@${npu_server_list[$name]}:/home/zkjh
    ...
    while true; do
        ...
        if [ ${#servers[@]} -ge ${SERVER_QUANTITY} ]; then
            ...
            if [ $TEST_TYPE == "Inference" ]; then
                ...
                $curr_dir/infiniLM_metax_test.sh 1 "${servers[*]}" ${model} 0 ${TEST_TYPE} ${ENGINE_TYPE} ${SESSION_ID} "${TEST_PARAM}" ${version} > $inference_log 2>&1 &
            else
                ...
                $curr_dir/infiniLM_metax_test.sh 1 "${servers[*]}" ${model} 0 ${TEST_TYPE} ${ENGINE_TYPE} ${SESSION_ID} "${TEST_PARAM}" ${version} > $log_path 2>&1 &
            fi
            last_pid=$!
    ...
    while true; do
        ...
        for item in "${GPU_resource_demand[@]}"; do
            ...
            if [ ${#servers[@]} -ge ${SERVER_QUANTITY} ]; then
                ...
                echo
                $curr_dir/infiniLM_metax_test.sh 0 "${servers[*]}" ${item} ${job_count} ${TEST_TYPE} ${ENGINE_TYPE} ${SESSION_ID} "${TEST_PARAM}" ${version} > $curr_dir/logs/service/$SESSION_ID/cron_job_${TEST_PARAM// /_}_${log_name_suffix}_${job_count}.log 2>&1 &
                last_pid=$!
