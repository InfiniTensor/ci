...
if [ -z $VERSION ]; then
    ...
    TAGS=$(/home/zkjh/jfrog rt curl \
        --server-id=my-jcr \
        /api/docker/docker-local/v2/infiniLM-aarch64-metax/tags/list \
    | jq -r '.tags[]' | sort)
    ...
    created=$(/home/zkjh/jfrog rt curl \
        --server-id=my-jcr \
        /api/storage/docker-local/infiniLM-aarch64-metax/$tag \
        | jq -r '.created')
    echo "$tag $created"
    done > tag_dates.txt
...
ret=`docker ps -a | grep infiniLM_metax_<<<TEST_TYPE>>>_${MODEL}_${OPTIONS}_${SESSION_ID}_${JOB_COUNT}`
if [ $? -eq 0 ]; then
  docker stop infiniLM_metax_<<<TEST_TYPE>>>_${MODEL}_${OPTIONS}_${SESSION_ID}_${JOB_COUNT}
  docker rm infiniLM_metax_<<<TEST_TYPE>>>_${MODEL}_${OPTIONS}_${SESSION_ID}_${JOB_COUNT}
fi
...
if ! command -v mx-smi &> /dev/null; then
    echo "错误: mx-smi 未找到，请确保 metax C550 驱动已安装"
    exit 1
fi
...
EXEC_COMMAND="docker run --name=infiniLM_metax_<<<TEST_TYPE>>>_${MODEL}_${OPTIONS}_${SESSION_ID}_${JOB_COUNT} "
