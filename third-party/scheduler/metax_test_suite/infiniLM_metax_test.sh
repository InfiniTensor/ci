...
declare -A npu_server_list=(
    ["172.22.162.49"]="AICC_001"
)

declare -A local_ip_map=(
    ["172.22.162.49"]="192.168.162.49"
)
...
cleanup_all_resources() {
    ...
    echo ""
    echo "=========================================="
    echo "${engine_type}_metax_test.sh 退出，开始清理资源..."
    echo "=========================================="
    ...
    for ip in ${server_list[@]}; do
        ssh -q -o ConnectionAttempts=3 zkjh@$ip "
            name=${engine_type}_metax_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count}
...
if [ $TEST_TYPE == "Service" ]; then
    ...
    if [ -z $version ]; then
        version=$(jfrog rt curl --server-id=my-jcr /api/docker/docker-local/v2/infiniLM-aarch64-metax/tags/list | jq -r '.tags[]' \
                            | xargs -I% sh -c "echo -n \"%  \"; \
                                jfrog rt curl --server-id=my-jcr \
                                /api/storage/docker-local/infiniLM-aarch64-metax/% \
    ...
    for item in "${model_list[@]}"; do
                ...
                if [ $err -ne 0 ]; then
                    ...
                    for ip in ${server_list[@]}; do
                        if [ $ENGINE_TYPE == "InfiniLM" ]; then
                            ssh -q -o ConnectionAttempts=3  zkjh@$ip docker stop infiniLM_metax_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count}
                            ssh -q -o ConnectionAttempts=3 zkjh@$ip docker rm infiniLM_metax_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count}
                        elif [ $ENGINE_TYPE == "vLLM" ]; then
                            ssh -q -o ConnectionAttempts=3 zkjh@$ip docker stop vllm_metax_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count}
                            ssh -q -o ConnectionAttempts=3 zkjh@$ip docker rm vllm_metax_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count}
                        fi
                    done
        ...
        if [ $TEST_TYPE == "Service" ]; then
            ssh -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 zkjh@$local_master_ip "
                    docker exec ${ENGINE_TYPE,}_metax_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count} /bin/bash -c \"
            ...
            echo "docker run --rm --name $container_name --volume /data/shared/limingge/CI_Workspace_for_InfiniLM/ci_autotest/third-party/scheduler/metax_test_suite/report_${log_name_suffix}/$session_id:/test/report_${log_name_suffix} -e TASK_START_TIME=${log_name_suffix} --entrypoint /test/start.sh openai:1110 --file $filename --email limingge@xcoresigma.com --env=${npu_server_list[${server_list[0]}]} --url http://${server_list[0]}:${server_port}/v1 --model=$model_name --gpu 910B --cmd \"$full_cmd\""
            docker run --rm --name $container_name --volume /data/shared/limingge/CI_Workspace_for_InfiniLM/ci_autotest/third-party/scheduler/metax_test_suite/report_${log_name_suffix}/$session_id:/test/report_${log_name_suffix} -e TASK_START_TIME=${log_name_suffix} --entrypoint /test/start.sh openai:1110 --file $filename --email limingge@xcoresigma.com --env=${npu_server_list[${server_list[0]}]} --url http://${server_list[0]}:${server_port}/v1 --model=$model_name --gpu 910B --cmd "\"$full_cmd\"" 2>&1 &
            pid=$!
        ...
        if [ $send_report -eq 1 ]; then
            ...
            if [ $TEST_TYPE == "Smoke" ]; then
                ...
                CI_report_folder="/artifacts/CI_metax_test/${session_id}_metax_npu_performancetest"
