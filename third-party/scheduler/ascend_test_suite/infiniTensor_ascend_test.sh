...
declare -A npu_server_list=(
    ["172.22.162.8"]="AICC_001"
)

declare -A local_ip_map=(
    ["172.22.162.8"]="192.168.162.8"
)
...
if [ $TEST_TYPE == "Service" ]; then    # Web服务型测试任务
    ...
    if [ -z $version ]; then
        version=$(jfrog rt curl --server-id=my-jcr /api/docker/docker-local/v2/infiniTensor-aarch64-ascend/tags/list | jq -r '.tags[]' \
                            | xargs -I% sh -c "echo -n \"%  \"; \
                                jfrog rt curl --server-id=my-jcr \
                                /api/storage/docker-local/infiniTensor-aarch64-ascend/% \
                ...
                if [ $err -ne 0 ]; then
                    ...
                    for ip in ${server_list[@]}; do
                        if [ $ENGINE_TYPE == "InfiniTensor" ]; then
                            ssh -q -o ConnectionAttempts=3 -p 14735 zkjh@$ip docker stop infiniTensor_ascend_${TEST_TYPE}Test_${session_id}_${job_count}
                            ssh -q -o ConnectionAttempts=3 -p 14735 zkjh@$ip docker rm infiniTensor_ascend_${TEST_TYPE}Test_${session_id}_${job_count}
                        elif [ $ENGINE_TYPE == "vLLM" ]; then
                            ssh -q -o ConnectionAttempts=3 -p 14735 zkjh@$ip docker stop vllm_ascend_${TEST_TYPE}Test_${session_id}_${job_count}
                            ssh -q -o ConnectionAttempts=3 -p 14735 zkjh@$ip docker rm vllm_ascend_${TEST_TYPE}Test_${session_id}_${job_count}
                        elif [ $ENGINE_TYPE == "MindIE" ]; then
                            ssh -q -o ConnectionAttempts=3 -p 14735 zkjh@$ip docker stop mindie_ascend_${TEST_TYPE}Test_${session_id}_${job_count}
                            ssh -q -o ConnectionAttempts=3 -p 14735 zkjh@$ip docker rm mindie_ascend_${TEST_TYPE}Test_${session_id}_${job_count}
                        fi
                    done
    ...
    ssh -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 -p 14735 zkjh@$ip chmod a+x /home/zkjh/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test.sh
    ssh -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 -p 14735 zkjh@$ip /home/zkjh/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test.sh $model $gpu_quantity $server_list_str 0 0 $session_id $version > "$curr_dir/logs/unit/$session_id/${filename}" &
    ssh_pid=$!
    pid_map[$ssh_pid]=$ip
    SSH_PID_MAP[$ssh_pid]=$ip
    ...
    for ip in ${server_list[@]}; do
        if [ $ENGINE_TYPE == "InfiniTensor" ]; then
            ssh -q -o ConnectionAttempts=3 -p 14735 zkjh@$ip docker stop infiniTensor_ascend_${TEST_TYPE}Test_${session_id}_0
            ssh -q -o ConnectionAttempts=3 -p 14735 zkjh@$ip docker rm infiniTensor_ascend_${TEST_TYPE}Test_${session_id}_0
        fi
    done
    ...
    if [ $TEST_TYPE == "Unit"]
        ...
        echo "Inference Test Result..."
        # ...
    elif [ $TEST_TYPE == "Bench" ]; then
        echo "Bench Test Result..."
        # ...
    elif [ $TEST_TYPE == "Accuracy" ]; then
        echo "Accuracy Test Result..."
        # ...
    fi
fi
