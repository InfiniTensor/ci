...
if [ $TEST_TYPE == "Unit" ]; then
    ...
    for ip in ${server_list[@]}; do
        if [ $ENGINE_TYPE == "InfiniTensor" ]; then
            ssh -q -o ConnectionAttempts=3 -p 14735 zkjh@$ip docker stop infiniTensor_ascend_UnitTest_${session_id}_0
            ssh -q -o ConnectionAttempts=3 -p 14735 zkjh@$ip docker rm infiniTensor_ascend_UnitTest_${session_id}_0
        fi
    done
...
for item in "${model_list[@]}"; do
    ...
    if [ $TEST_TYPE == "Smoke" ]; then
        ...
        # echo "docker run --rm --name $container_name --volume /home/zkjh/CI_Workspace/ci_autotest/third-party/scheduler/ascend_test_suite/report_${log_name_suffix}/$session_id:/test/report_${log_name_suffix} -e TASK_START_TIME=${log_name_suffix} --entrypoint /test/start.sh openai:1110 --file $filename --email limingge@xcoresigma.com --env=${npu_server_list[${server_list[0]}]} --url http://${server_list[0]}:${server_port}/v1 --model=$model_name --gpu 910B --cmd \"$full_cmd\""
        # DOCKER_API_VERSION=1.39 docker run --rm --name $container_name --volume /home/zkjh/CI_Workspace/ci_autotest/third-party/scheduler/ascend_test_suite/report_${log_name_suffix}/$session_id:/test/report_${log_name_suffix} -e TASK_START_TIME=${log_name_suffix} --entrypoint /test/start.sh openai:1110 --file $filename --email limingge@xcoresigma.com --env=${npu_server_list[${server_list[0]}]} --url http://${server_list[0]}:${server_port}/v1 --model=$model_name --gpu 910B --cmd "\"$full_cmd\"" 2>&1 &
        echo "docker run --rm --name $container_name --ulimit nproc=8192:8192 --volume /home/zkjh/CI_Workspace/ci_autotest/third-party/scheduler/ascend_test_suite/report_${log_name_suffix}/$session_id:/test/report_${log_name_suffix} -e TASK_START_TIME=${log_name_suffix} -e OPENBLAS_NUM_THREADS=1 -e OMP_NUM_THREADS=1 -e MKL_NUM_THREADS=1 -e NUMEXPR_NUM_THREADS=1 -e JAVA_HOME=/usr/lib/jvm/java-11-openjdk -e PYTHONUNBUFFERED=1 --entrypoint /test/start.sh openai:1110 --file $filename --email limingge@xcoresigma.com --env=${npu_server_list[${server_list[0]}]} --url http://${server_list[0]}:${server_port}/v1 --model=$model_name --gpu 910B --cmd \"$full_cmd\""
        DOCKER_API_VERSION=1.39 docker run --rm --name $container_name --ulimit nproc=8192:8192 --volume /home/zkjh/CI_Workspace/ci_autotest/third-party/scheduler/ascend_test_suite/report_${log_name_suffix}/$session_id:/test/report_${log_name_suffix} -e TASK_START_TIME=${log_name_suffix} -e OPENBLAS_NUM_THREADS=1 -e OMP_NUM_THREADS=1 -e MKL_NUM_THREADS=1 -e NUMEXPR_NUM_THREADS=1 -e JAVA_HOME=/usr/lib/jvm/java-11-openjdk -e PYTHONUNBUFFERED=1 --entrypoint /test/start.sh openai:1110 --file $filename --email limingge@xcoresigma.com --env=${npu_server_list[${server_list[0]}]} --url http://${server_list[0]}:${server_port}/v1 --model=$model_name --gpu 910B --cmd "\"$full_cmd\"" 2>&1 &
        pid=$!
