...
if [ -z $VERSION ]; then
    ...
    TAGS=$(/home/zkjh/jfrog rt curl \
        --server-id=my-jcr \
        /api/docker/docker-local/v2/infiniTensor-aarch64-ascend/tags/list \
    | jq -r '.tags[]' | sort)
    ...
    created=$(/home/zkjh/jfrog rt curl \
        --server-id=my-jcr \
        /api/storage/docker-local/infiniTensor-aarch64-ascend/$tag \
        | jq -r '.created')
    echo "$tag $created"
    done > tag_dates.txt
...
if [ $? -eq 0 ]; then
  docker stop infiniTensor_ascend_<<<TEST_TYPE>>>_${SESSION_ID}_${JOB_COUNT}
  docker rm infiniTensor_ascend_<<<TEST_TYPE>>>_${SESSION_ID}_${JOB_COUNT}
fi
