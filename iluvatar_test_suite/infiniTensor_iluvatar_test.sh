    ...
    while (( remaining > 0 )); do
        ...
        if [ -v pid_map[$done_pid] ]; then
            ...
            if [ $err -ne 0 ]; then
                ...
                for ip in ${server_list[@]}; do
                    if [ $ENGINE_TYPE == "InfiniTensor" ]; then
                        ssh -q -o ConnectionAttempts=3 -p 14735 zkjh@$ip docker stop infiniTensor_iluvatar_${TEST_TYPE}Test_${session_id}_${job_count}
                        ssh -q -o ConnectionAttempts=3 -p 14735 zkjh@$ip docker rm infiniTensor_iluvatar_${TEST_TYPE}Test_${session_id}_${job_count}
                    elif [ $ENGINE_TYPE == "vLLM" ]; then
                        ssh -q -o ConnectionAttempts=3 -p 14735 zkjh@$ip docker stop vllm_iluvatar_${TEST_TYPE}Test_${session_id}_${job_count}
                        ssh -q -o ConnectionAttempts=3 -p 14735 zkjh@$ip docker rm vllm_iluvatar_${TEST_TYPE}Test_${session_id}_${job_count}
                    elif [ $ENGINE_TYPE == "MindIE" ]; then
                        ssh -q -o ConnectionAttempts=3 -p 14735 zkjh@$ip docker stop mindie_iluvatar_${TEST_TYPE}Test_${session_id}_${job_count}
                        ssh -q -o ConnectionAttempts=3 -p 14735 zkjh@$ip docker rm mindie_iluvatar_${TEST_TYPE}Test_${session_id}_${job_count}
                    fi
                done
