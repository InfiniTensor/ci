...
if [ $model == "DeepSeek-R1" ]; then
    ...
    echo "SIG_LOG_LEVEL='warn,console_logger=info' ./start.sh --schedule-policy $SCHEDULE_POLICY --master-port $MASTER_PORT --nnodes 1 --node-rank 0 --model QwQ-32B --tokenizer /home/weight/QwQ-32B -tp 2 --max-num-batched-tokens 8192 --gpu-memory-utilization 0.9 --port $PORT --prometheus-port $PROMETHEUS_PORT --platform-type nvidia $USE_PREFIX_CACHE $SWAP_SPACE_OPTION"
    EXEC_COMMAND+=" --schedule-policy $SCHEDULE_POLICY --master-port $MASTER_PORT --nnodes 1 --node-rank 0 --model QwQ-32B --tokenizer /home/weight/QwQ-32B -tp 2 --max-num-batched-tokens 8192 --gpu-memory-utilization 0.9 --port $PORT --prometheus-port $PROMETHEUS_PORT --platform-type nvidia $USE_PREFIX_CACHE $SWAP_SPACE_OPTION > $LOG_NAME 2>&1 &"
    ...
    echo "SIG_LOG_LEVEL='warn,console_logger=info' ./start.sh --schedule-policy $SCHEDULE_POLICY --master-port $MASTER_PORT --nnodes 1 --node-rank 0 --model QwQ-32B-AWQ --tokenizer /home/weight/QwQ-32B-AWQ -tp 1 --weight-dtype=FP16 --max-num-batched-tokens 8192 --gpu-memory-utilization 0.9 --port $PORT --prometheus-port $PROMETHEUS_PORT --platform-type nvidia $USE_PREFIX_CACHE $SWAP_SPACE_OPTION"
    EXEC_COMMAND+=" --schedule-policy $SCHEDULE_POLICY --master-port $MASTER_PORT --nnodes 1 --node-rank 0 --model QwQ-32B-AWQ --tokenizer /home/weight/QwQ-32B-AWQ -tp 1 --weight-dtype=FP16 --max-num-batched-tokens 8192 --gpu-memory-utilization 0.9 --port $PORT --prometheus-port $PROMETHEUS_PORT --platform-type nvidia $USE_PREFIX_CACHE $SWAP_SPACE_OPTION > $LOG_NAME 2>&1 &"
