...
curr_dir=/home/s_limingge/accuracy_test_ascend
            ...
            for item in "${model_list[@]}"; do
                ...
                while (( remaining > 0 )); do
                    ...
                    if [ $err -ne 0 ]; then
                        ...
                        for ip in ${server_list[@]}; do
                            if [ $ip == "10.9.1.6" ]; then
                                sshpass -p 's_limingge' ssh -o ConnectionAttempts=3 s_limingge@10.9.1.6 docker stop siginfer_ascend_AccuracyTest_${job_count}
                                sshpass -p 's_limingge' ssh -o ConnectionAttempts=3 s_limingge@10.9.1.6 docker rm siginfer_ascend_AccuracyTest_${job_count}
                            else
                                ssh -o ConnectionAttempts=3 s_limingge@$ip docker stop siginfer_ascend_AccuracyTest_${job_count}
                                ssh -o ConnectionAttempts=3 s_limingge@$ip docker rm siginfer_ascend_AccuracyTest_${job_count}
                            fi
                        done
                        success=1
                        break
